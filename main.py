...
async def get_user_by_order(order_id) -> dict | None:
    ...
    try:
        uuid.UUID(order_str)
        user = await database.fetch_one(
            "SELECT id FROM users WHERE id=CAST(:oid AS UUID) AND is_active=TRUE",
            {"oid": order_str}
        )
        return user
    except ValueError:
...
async def create_transaction(req_id, params):
    ...
    tx = await database.fetch_one(
        """INSERT INTO payme_transactions
           ...
           VALUES (:pid, CAST(:uid AS UUID), :amt, 1, :ct)
           RETURNING *""",
...
async def perform_transaction(req_id, params):
    ...
    async with database.transaction():
        await database.execute(
            "UPDATE wallets SET balance=balance+:a, updated_at=NOW() WHERE user_id=CAST(:uid AS UUID)",
            {"a": amount_uzs, "uid": user_id_str}
        )
        ledger_tx = await database.fetch_one(
            """INSERT INTO transactions
               (receiver_id, amount, type, status, description, reference)
               VALUES (CAST(:uid AS UUID), :a, 'topup', 'completed', 'Payme orqali toldirish', :ref)
               RETURNING id""",
